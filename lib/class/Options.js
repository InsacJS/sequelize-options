...
class Options {
  ...
  * @param {Object} [options.query] Filtros de una consulta.
  * @param {Object} [options.output] Formato de salida.
  * @param {Boolean} [options.plain=false] Indica si los campos de los submodelos se moverán al objeto raiz.
  * los campos que sean claves primarias y foráneas.
  * @return {Object}
  */
  ...
  if (Array.isArray(OPT.attributes)) {
    OPT.attributes.forEach(attrib => {
      ...
      if (QUERY[path]) {
        ...
        const value = (QUERY[path].indexOf(',') !== -1) ? QUERY[path].split(',') : QUERY[path]
...
* @param {Object} attributes Propiedad attributes.
* @param {Object} [OUTPUT] Objeto que contiene el formato.
* @param {SequelizeModel} [MODEL] Modelo Sequelize de referencia.
* @param {Boolean} KEYS Indica si se incluiran por defecto las claves primarias y foraneas.
* @return {Object}
*/
function normalizeAttributes (attributes, OUTPUT, MODEL, KEYS) {
  const exclude = (attributes && attributes.exclude) ? attributes.exclude : []
  let ATTRIBUTES = (attributes && Array.isArray(attributes)) ? attributes : []
  if (!KEYS) {
    if (exclude.length > 0) {
      if (OUTPUT) {
        for (let prop in OUTPUT) {
